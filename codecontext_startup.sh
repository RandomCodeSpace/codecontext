...
if pgrep -f "python -m codecontext .*mcp -http|codecontext mcp -http" >/dev/null 2>&1; then
	echo "$(date +"%Y-%m-%dT%H:%M:%S") - Stopping existing codecontext mcp -http processes" >> "$LOG_FILE"
	...
	if pgrep -f "python -m codecontext .*mcp -http|codecontext mcp -http" >/dev/null 2>&1; then
		echo "$(date +"%Y-%m-%dT%H:%M:%S") - Forcing kill of remaining processes" >> "$LOG_FILE"
		pkill -9 -f "python -m codecontext .*mcp -http|codecontext mcp -http" || true
	fi
fi

echo "$(date +"%Y-%m-%dT%H:%M:%S") - Starting codecontext mcp -http" >> "$LOG_FILE"
